...
(function(document, $, undefined) {
    ...
    Date.prototype.toUTCDate = function() {
        return this.getUTCFullYear() + '-' + padZero(this.getUTCMonth()+1) + '-' + padZero(this.getUTCDate());
    };
    ...
    Date.prototype.toDateTime = function() {
        var hour = padZero(this.getHours()),
            min = padZero(this.getMinutes()),
            sec = padZero(this.getSeconds());
    ...
    Date.prototype.toUTCDateTime = function() {
        var hour = padZero(this.getUTCHours()),
            min = padZero(this.getUTCMinutes()),
            sec = padZero(this.getUTCSeconds());
    ...
    helper.date = {
         ...
         */
        sqlTimeToTimestamp: function(time, milliSecond) {
            if (!time) {
        ...
        /*sqlDateToHuman: function(date) {
         return date.replace(/(\d{4})-(\d{2})-(\d{2})/, function(match, year, month, day) {
         month = this.monthNumberToString(month, 'short');
        ...
        monthStringToNumber: function(month) {
            for (var i in monthFormat.short) {
                if (new RegExp('^' + monthFormat.short[i] + '[a-z]*$', 'i').test(month)) {
        ...
        monthNumberToString: function(month, format) {
            ...
            for (var i in monthFormat.number) {
